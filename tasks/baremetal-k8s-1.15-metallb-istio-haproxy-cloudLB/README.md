...
## Gateway istio-autogenerated-k8s-ingress is configured for any header host for http/https protocol ingress control by default.  To restrict incoming request header host, specifying "example.com" to Hosts in yaml
```kubectl describe gateway/istio-autogenerated-k8s-ingress -n istio-system```

## Create Istio virtualservice.  uri prefix "/" accepts any request that beginning with "/", route defines which service and port to send request to
```
cat <<EOF | kubectl apply -f -
