...
## Create Istio gateway for http protocol ingress control
## "*" accepts all coming http requests with any Host header
## If you specify "a.com" meaning that gateway only accepts http requests with header Host="a.com", make sure you config haproxy forwarding metadata
```
cat <<EOF | kubectl apply -f -
apiVersion: networking.istio.io/v1alpha3
kind: Gateway
metadata:
  name: istio-http-gw-ingress
  namespace: istio-system
  labels:
    app: ingressgateway
spec:
